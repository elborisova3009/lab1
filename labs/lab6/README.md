...
*Магистральный канал позволяет передавать трафик нескольких VLAN через один канал. Без магистрального порта для каждой VLAN требуется отдельное соединение между коммутаторами.*
